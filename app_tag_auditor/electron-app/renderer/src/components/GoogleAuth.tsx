...
export default function GoogleAuth({ clientId, redirectUri, label = 'Connect Google Account', fullWidth }: GoogleAuthProps) {
  if (!clientId) {
    return (
      <button
        ...
        onClick={() => alert('Google Client ID is not configured. Check your .env file.')}
  ...
  return (
    <a
      ...
      className="btn-google"
      style={fullWidth ? { width: '100%', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none' } : { display: 'inline-flex', alignItems: 'center', textDecoration: 'none' }}
    >
      <svg viewBox="0 0 24 24" width="18" height="18">
        <path
          fill="#EA4335"
